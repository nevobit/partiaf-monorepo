...
export const createOrderExternal = async ({
  title,
  price,
  email,
  amount,
  ...
  const ticket =
    price < 1500000
      ? "adf6f9f8-22c0-4673-a513-40cadd254f23"
      : "7da7fe52-b0a6-44b8-9570-86da819834ba";
  const goer = new model({
    ticket,
...
export const updatePaymentExternal = async (payment: any, id: string) => {
  ...
  if (payment.type === "payment") {
    ...
    if (data.body.status == "approved") {
      ...
      const cover = await modelCover.findById(goer?.ticket);
      ...
      cover.limit = cover.limit - goer?.amount!;

      if (!store) return new Error(`No store found`);
      store.balance = store.balance + goer?.cost!;
      goer.isPaid = true;
