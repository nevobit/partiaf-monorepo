...
const RegisterBusiness = () => {    
    ...
    const {success: successStore} = useSelector((state:AppStore) => state.stores);
        ...
        <div className={styles.main}>
            ...
            <form className={styles.form} onSubmit={onSubmit}>
                ...
                <div className={styles.inputs}>
                    ...
                    <Field label="Cupo total" tip='Indica cuantas personas caben en tu establecimiento'>
                        <Input
                            required
                            type="number"
                            name="limit"
                            id="limit"
                            placeholder="Ingresa el cupo total"
                            onChange={handleChange}
                        />
                    </Field>
                    <Field label="Codigo Colaboradores" tip="Inserte un codigo de 4 digitos para que sus colaboradores puedas usar nuestra app" >
                        <Input
                            required
                            variant='number'
                            type="number"
 ...
 type='button'
 loading={loading}
 variant='dark'><Link to={PublicRoutes.SIGNIN}>Cancelar</Link></Button>
                ) : (
                    <Button
                    type='button'
                    onClick={()=> setScreen(0)}
                    variant='dark'>Atras</Button>
                )}
           {screen == 0? (
            <Button type='button' loading={loading} onClick={() => setScreen(1)}>Siguiente</Button>

           ): (
            <Button type='button' loading={loading} onClick={onSubmit}>Siguiente</Button>
           )}
            </div>
            </form>
        </div>
